...
WAITING_GIF_GOAL = 11
WAITING_GIF_SAVE = 12
WAITING_MEME = 13  # новое состояние
...
# ---------- Функции для мемов ----------
def add_meme(file_id):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('INSERT INTO memes (file_id) VALUES (?)', (file_id,))
    conn.commit()
    conn.close()
...
def get_random_meme():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('SELECT file_id FROM memes')
    rows = c.fetchall()
    conn.close()
    if not rows:
        return None
...
async def freegoyda_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if meme is None:
        await update.message.reply_text("😅 Мемов пока нет, загрузите их в админке.")
    else:
...
async def admin_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "➕ Добавить каналы":
        ...
        return ConversationHandler.END
    elif text == "🖼 Добавить мем":
        context.user_data["in_conversation"] = True
        await update.message.reply_text("📤 Отправьте **фотографию** (картинку) для мема.\nДля отмены отправьте /cancel")
        return WAITING_MEME
    elif text == "🚪 Выйти":
...
async def receive_meme(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Сохраняет присланную фотографию как мем."""
    message = update.message
    if not message.photo:
        await update.message.reply_text("❌ Пожалуйста, отправьте **фотографию** (картинку).")
        return WAITING_MEME

    # Берём самое качественное фото (последний элемент в списке)
    file_id = message.photo[-1].file_id
    add_meme(file_id)
    await update.message.reply_text("✅ Мем добавлен!", reply_markup=admin_menu_keyboard())
    context.user_data["in_conversation"] = False
    return ConversationHandler.END

# ---------- Обработчик текста на мемы ----------
async def handle_meme_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Проверяет текст на наличие ключевых слов 'мэмас' или 'мемасек' и отправляет мем."""
    # Не обрабатываем, если пользователь в диалоге
    if context.user_data.get("in_conversation", False):
        return
    text = update.message.text.lower()
    if "мэмас" in text or "мемасек" in text:
        meme = get_random_meme()
        if meme is None:
            await update.message.reply_text("😅 Мемов пока нет, загрузите их в админке.")
        else:
...
async def handle_unknown_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in ["🏠 Главное меню", "➕ Добавить каналы", "➕ Добавить чаты",
                "📩 Проверить поддержку", "⚙️ Настройки", "🎮 Настройки игры", "🖼 Добавить мем", "🚪 Выйти"]:
        return
...
def main():
    ...
    conv_gif_goal = ConversationHandler(
        ...
        fallbacks=[CommandHandler("cancel", cancel_gif_dialog)],
        allow_reentry=True,
    )
    ...
    # Диалог добавления мема
    conv_meme = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^🖼 Добавить мем$") & filters.ChatType.PRIVATE, admin_buttons)],
        states={
            WAITING_MEME: [MessageHandler(filters.PHOTO, receive_meme)],
        },
        fallbacks=[CommandHandler("cancel", lambda u,c: u.message.reply_text("Отменено."))],
        allow_reentry=True,
    )
    app.add_handler(conv_meme)
    ...
    # Обработчик текста на мемы (до автоудаления)
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_meme_text), group=1)

    # Автоудаление неизвестных сообщений (после всех обработчиков)
    ...
    app.add_handler(CommandHandler("freegoyda", freegoyda_command))  # новая команда
