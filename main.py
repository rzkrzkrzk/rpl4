...
async def send_meme_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    meme_file_id = get_setting("meme_file_id")
    if meme_file_id:
        await update.message.reply_photo(photo=meme_file_id)
    else:
        await update.message.reply_text("⚠️ Мем еще не был добавлен администратором.")
...
async def admin_buttons_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif text in ["🖼 Добавить мем", "Добавить мем"]:
        await update.message.reply_text("Отправьте картинку (фотографию), которая будет использоваться как мем:")
        return ADD_MEME

# --- Добавление мема ---
async def process_add_meme(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.photo:
        file_id = update.message.photo[-1].file_id
        set_setting("meme_file_id", file_id)
        await update.message.reply_text("✅ Мем успешно сохранён!", reply_markup=admin_reply_keyboard())
        return ConversationHandler.END
    else:
        await update.message.reply_text("❌ Отправьте именно картинку (фотография).")
        return ADD_MEME
...
def main():
    ...
    add_meme_handler = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^(🖼 Добавить мем|Добавить мем)$"), admin_buttons_handler)],
        states={
            ADD_MEME: [MessageHandler(filters.PHOTO | (filters.TEXT & ~filters.COMMAND), process_add_meme)],
        },
        fallbacks=[CommandHandler("cancel", cancel_handler)]
    )

    app.add_handler(CommandHandler("start", start_command))
    app.add_handler(CommandHandler("freegoyda", send_meme_handler))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.Regex(r"(?i)^(мэмас|мемасек)$"), send_meme_handler))
